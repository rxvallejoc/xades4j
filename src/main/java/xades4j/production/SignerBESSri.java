...
import com.obiectumclaro.DatosUsuario;
import com.obiectumclaro.LeerCertificado;
 ...
 * @author Luís
 */
public class SignerBESSri implements XadesSigner {
	...
	protected SignerBESSri(
		...
		if (ObjectUtils.anyNull(keyingProvider, algorithmsProvider,
				signaturePropsProvider, dataObjPropsProvider,
				propsDataObjectsGenerator, signedPropsMarshaller,
				unsignedPropsMarshaller, algorithmsParametersMarshaller)) {
		...
		this.keyInfoBuilder = new KeyInfoBuilder(basicSignatureOptionsProvider,
				algorithmsProvider);
		this.qualifPropsProcessor = new QualifyingPropertiesProcessor(
				signaturePropsProvider, dataObjPropsProvider);
		this.datosUsuario = new DatosUsuario();
	}

	@Override
	public final XadesSignatureResult sign(SignedDataObjects signedDataObjects,
			Node parent) throws XAdES4jException {
		return sign(signedDataObjects, parent,
				SignatureAppendingStrategies.AsLastChild);
	}

	@Override
	public final XadesSignatureResult sign(SignedDataObjects signedDataObjects,
			Node referenceNode, SignatureAppendingStrategy appendingStrategy)
		...
		List<X509Certificate> signingCertificateChain = this.keyingProvider
				.getSigningCertificateChain();

		if (null == signingCertificateChain
				|| signingCertificateChain.isEmpty()) {
			throw new SigningCertChainException(
					"Signing certificate not provided");
		}
		X509Certificate signingCertificate = signingCertificateChain.get(0);
		try {
			LeerCertificado certificadoInfo = new LeerCertificado(new String[] {
					"1.2.3.4", "1.3.6.1.4.1.37947.3", "1.3.6.1.4.1.37746.3",
					"1.2.840.113549.1.1.5" });
			this.datosUsuario = certificadoInfo
					.crearDatosUsuario(signingCertificate);
		} catch (Exception ex) {
			throw new SigningCertChainException(ex.getMessage());
		}
		// The XMLSignature (ds:Signature).
		XMLSignature signature = createSignature(signatureDocument,
				signedDataObjects.getBaseUri(), signingCertificate
		...
		Map<DataObjectDesc, Reference> referenceMappings = this.dataObjectDescsProcessor
		...
		Element qualifyingPropsElem = ElementProxy.createElementForFamily(
				signature.getDocument(), QualifyingProperty.XADES_XMLNS,
				QualifyingProperty.QUALIFYING_PROPS_TAG);
		qualifyingPropsElem.setAttributeNS(null,
				QualifyingProperty.TARGET_ATTR, '#' + signatureId);
		...
		ObjectContainer qPropsXmlObj = new ObjectContainer(
		...
		Collection<SignedSignatureProperty> fsssp = new ArrayList<SignedSignatureProperty>(
				2);
		Collection<UnsignedSignatureProperty> fsusp = new ArrayList<UnsignedSignatureProperty>(
				2);
		getFormatSpecificSignatureProperties(fsssp, fsusp,
				signingCertificateChain);
		// Gather all the signature and data objects properties.
		QualifyingProperties qualifProps = qualifPropsProcessor
				.getQualifyingProperties(signedDataObjects, fsssp, fsusp);
		...
		try {
			...
			PropertiesDataGenerationContext propsDataGenCtx = new PropertiesDataGenerationContext(
					signedDataObjects.getDataObjectsDescs(), referenceMappings,
					signatureDocument);
			...
			SigAndDataObjsPropertiesData signedPropsData = this.propsDataObjectsGenerator
					.generateSignedPropertiesData(
							qualifProps.getSignedProperties(), propsDataGenCtx);
			// Marshal the signed properties data to the QualifyingProperties
			// node.
			this.signedPropsMarshaller.marshal(signedPropsData,
					qualifyingPropsElem);
			Element signedPropsElem = DOMHelper
					.getFirstChildElement(qualifyingPropsElem);
			DOMHelper.setIdAsXmlIdSri(signedPropsElem, signedPropsId);
			...
			String digestAlgUri = algorithmsProvider
					.getDigestAlgorithmForDataObjsReferences();
			if (null == digestAlgUri) {
				throw new NullPointerException(
						"Digest algorithm URI not provided");
			}

			try {
				signature.addDocument('#' + signedPropsId, null, digestAlgUri,
						null, QualifyingProperty.SIGNED_PROPS_TYPE_URI);
			} catch (XMLSignatureException ex) {
			...
			// Apply the signature
			try {
				PrivateKey signingKey = keyingProvider
						.getSigningKey(signingCertificate);
			...
			Element sigValueElem = DOMHelper.getFirstDescendant(
					signature.getElement(), Constants.SignatureSpecNS,
					Constants._TAG_SIGNATUREVALUE);
			DOMHelper.setIdAsXmlIdSri(sigValueElem,
	...
	private XMLSignature createSignature(Document signatureDocument,
			String baseUri, String signingKeyAlgorithm)
			throws XAdES4jXMLSigException, UnsupportedAlgorithmException {
		Algorithm signatureAlg = this.algorithmsProvider
				.getSignatureAlgorithm(signingKeyAlgorithm);
		if (null == signatureAlg) {
			throw new NullPointerException("Signature algorithm not provided");
		}
		Element signatureAlgElem = createElementForAlgorithm(signatureAlg,
				Constants._TAG_SIGNATUREMETHOD, signatureDocument);

		Algorithm canonAlg = this.algorithmsProvider
				.getCanonicalizationAlgorithmForSignature();
		if (null == canonAlg) {
			throw new NullPointerException(
					"Canonicalization algorithm not provided");
		}
		Element canonAlgElem = createElementForAlgorithm(canonAlg,
				Constants._TAG_CANONICALIZATIONMETHOD, signatureDocument);

		try {
			return new XMLSignature(signatureDocument, baseUri,
					signatureAlgElem, canonAlgElem);
	...
	private Element createElementForAlgorithm(Algorithm algorithm,
			String elementName, Document signatureDocument)
			throws UnsupportedAlgorithmException {
		Element algorithmElem = XMLUtils.createElementInSignatureSpace(
				signatureDocument, elementName);
		algorithmElem.setAttributeNS(null, Constants._ATT_ALGORITHM,
				algorithm.getUri());

		List<Node> algorithmParams = this.algorithmsParametersMarshaller
				.marshalParameters(algorithm, signatureDocument);
	...
	protected void getFormatSpecificSignatureProperties(
			...
			throws XAdES4jException {
		SigningCertificateProperty scp = new SigningCertificateProperty(
				signingCertificateChain);
