...
public class SignerBESTest extends SignerTestBase {
	...
	public void testCert() {
		...
		try {
			keyingProviderMy = createFileSystemKeyingDataProvider("pkcs12",
					"my/mponce.crt", "passwor", true);
		} catch (KeyStoreException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		try {
			SignerBESSri signer = (SignerBESSri) new XadesBesSigningProfileSri(
					keyingProviderMy).newSigner();
	...
	@Test
	public void testSignBESSri() throws Exception {
		System.out.println("signBESSri");

		Document doc1 = getTestDocumentSri();
		Element elemToSign = doc1.getDocumentElement();
		SignerBESSri signer = (SignerBESSri) new XadesBesSigningProfileSri(
				keyingProviderMy).newSigner();

		AllDataObjsCommitmentTypeProperty globalCommitment = AllDataObjsCommitmentTypeProperty
				.proofOfApproval();
		CommitmentTypeProperty commitment = CommitmentTypeProperty
				.proofOfCreation();
		DataObjectDesc obj1 = new DataObjectReference(
				'#' + elemToSign.getAttribute("id"))
				.withTransform(new EnvelopedSignatureTransform())
				.withDataObjectFormat(
						new DataObjectFormatProperty("text/xml", "UTF-8")
								.withDescription("Firma SRI"))
				.withCommitmentType(commitment);
		SignedDataObjects dataObjs = new SignedDataObjects(obj1)
				.withCommitmentType(globalCommitment);
		signer.sign(dataObjs, elemToSign);

		outputDocument(doc1, "document.signed.bes.sri.xml");
	}
	...
	public void testSignBES() throws Exception {
		...
		Element elemToSign = doc1.getDocumentElement();
		SignerBES signer = (SignerBES) new XadesBesSigningProfile(
				keyingProviderMy).newSigner();
		...
		DataObjectDesc obj1 = new DataObjectReference(
				...
				.withTransform(new EnvelopedSignatureTransform())
				.withDataObjectFormat(
						new DataObjectFormatProperty("text/xml", "MyEncoding")
								.withDescription(
										"Isto é uma descrição do elemento raiz")
		...
		DataObjectDesc obj2 = new EnvelopedXmlObject(objectContent, "text/xml",
				null)
				.withDataObjectFormat(
						new DataObjectFormatProperty("text/xml", "MyEncoding")
								.withDescription(
										"Isto é uma descrição do elemento dentro do object")
								.withDocumentationUri("http://doc3.txt")
								.withDocumentationUri("http://doc4.txt")
	...
	public void testSignBESExtrnlRes() throws Exception {
		...
		SignerBES signer = (SignerBES) new XadesBesSigningProfile(
				keyingProviderNist).newSigner();

		DataObjectDesc obj1 = new DataObjectReference("rfc3161.txt")
				.withDataObjectFormat(
						new DataObjectFormatProperty("text/plain")
								.withDescription("Internet X.509 Public Key Infrastructure Time-Stamp Protocol (TSP)"))
				.withDataObjectTimeStamp(
						new IndividualDataObjsTimeStampProperty());
		signer.sign(new SignedDataObjects(obj1)
				.withBaseUri("http://www.ietf.org/rfc/"), doc);
	...
	public void testSignBESWithCounterSig() throws Exception {
		...
		XadesBesSigningProfile profile = new XadesBesSigningProfile(
				keyingProviderMy);
		...
		profile.withSignaturePropertiesProvider(new SignaturePropertiesProvider() {
			...
			public void provideProperties(
				...
				signedPropsCol
						.addCounterSignature(new CounterSignatureProperty(
								counterSigner));
				signedPropsCol.setSignerRole(new SignerRoleProperty(
						"CounterSignature maniac"));
			}
		});
